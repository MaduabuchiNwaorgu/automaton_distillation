...
import random
import pickle, gzip
from utils import get_wasserstein, get_kl
from utils import AgentCollection,load_env_and_model
import numpy as np
from copy import deepcopy

class Student(object):
    def __init__(self, env, policy, memory, args, optimizer=None):
        ...
        self.policy = policy(num_inputs, num_actions, hidden_sizes=(args.hidden_size,) * args.num_layers) # to make this for my environment
        self.agents = AgentCollection([self.env], [self.policy], self.memory, args.device, render=args.render, num_agents=1)
        if not optimizer:
...
class Teacher(object):
    def __init__(self, envs, policies, args):
        ...
        self.agents = AgentCollection(self.envs, self.policies, args.device, num_agents=args.agent_count)
