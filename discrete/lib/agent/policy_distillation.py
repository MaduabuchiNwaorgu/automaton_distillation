...
from discrete.lib.agent.feature_extractor import StudentFeatureExtractor, FeatureExtractor


class TeacherDuelingQNetworkAgent(Agent):
    ...
    def create_target_agent(self) -> "TargetAgent":
        return EasyTargetAgent(self, TeacherDuelingQNetworkAgent(self.input_shape, self.num_actions)).to(
            self.adv_branch.weight.device)



class StudentDuelingQNetworkAgent(Agent):
    """A basic dueling q-agent"""

    def __init__(self, input_shape: Tuple, num_actions: int):
        super().__init__()
        self.input_shape = input_shape
        self.feature_extractor = StudentFeatureExtractor(input_shape=input_shape,pool_output_size=(10, 10))
        self.half_feat_extractor_output_size = self.feature_extractor.output_size // 2
        self.val_branch = nn.Linear(self.half_feat_extractor_output_size, 1)
        self.adv_branch = nn.Linear(self.half_feat_extractor_output_size, num_actions)
        self.flattener = nn.Flatten()
